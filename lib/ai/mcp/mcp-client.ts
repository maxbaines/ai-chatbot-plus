...
export async function initializeMCPClients(
  ...
  for (const mcpServer of mcpServers) {
    try {
      ...
      if (mcpServer.streaming) {
        // Use StreamableHTTPClientTransport for streaming servers
        transport = new StreamableHTTPClientTransport(
          new URL(mcpServer.url)
        );
      } else {
        // Use standard SSE transport
        transport = {
          type: 'sse' as const,
          url: mcpServer.url,
      ...
      console.log(`MCP tools from ${mcpServer.url} (streaming: ${mcpServer.streaming}):`, Object.keys(mcptools));
